...
class PlayerPreLoginEvent extends Event{
	...
	public const KICK_FLAG_CHEAT = 4;
	...
	public function hasEditionFaker(PlayerInfo $playerInfo) : bool{
		...
		if($extraData["DeviceOS"] === DeviceOS::ANDROID){
			if(is_string($extraData["DeviceModel"])){
		...
		if($extraData["DeviceOS"] === DeviceOS::IOS){
			if(is_string($extraData["DeviceId"])){
