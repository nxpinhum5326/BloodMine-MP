...
class Server{
	...
	public function __construct(
		...
		try{
			...
			$webhookYmlPath = Path::join($this->dataPath, "webhook.yml");
			...
			if(!file_exists($webhookYmlPath)){
				$content = Filesystem::fileGetContents(Path::join(\pocketmine\RESOURCE_PATH, "webhook.yml"));
				file_put_contents($webhookYmlPath, $content);
			}

			$this->configGroup = new ServerConfigGroup(
				new Config($webhookYmlPath, Config::YAML, []),
			...
			if($this->configGroup->getPropertyBool(Yml::SETTINGS_DISCORD_WEBHOOK_MESSAGE, false)){
