...
final class ServerConfigGroup{
	...
	public function __construct(
		private Config $webhookYml,
		private Config $pocketmineYml,
		private Config $serverProperties
	){}

	public function getWebhookSettings(string $variable, mixed $defaultValue = null) : mixed{
		if(!array_key_exists($variable, $this->propertyCache)){
			$v = getopt("", ["$variable::"]);
			if(isset($v[$variable])){
				$this->propertyCache[$variable] = $v[$variable];
			}else{
				$this->propertyCache[$variable] = $this->webhookYml->getNested($variable);
			}
		}
