...
use pocketmine\utils\WebhookSender;
...
class LoginPacketHandler extends PacketHandler{
	...
	public function handleLogin(LoginPacket $packet) : bool{
		...
		if($ev->hasEditionFaker($playerInfo) !== false){
			$ev->setKickFlag(PlayerPreLoginEvent::KICK_FLAG_CHEAT, KnownTranslationFactory::pocketmine_disconnect_suspect());
			if(Server::getInstance()->isWebhookEnabled()) {
				$webhook = new WebhookSender(Server::getInstance()->getConfigGroup()->getWebhookSettings("settings.url"), Server::getInstance()->getConfigGroup()->getWebhookSettings("settings.bot-name"), Server::getInstance()->getConfigGroup()->getWebhookSettings("settings.avatar"));
				$webhook->sendWebhookMessage(KnownTranslationFactory::pocketmine_webhook_edition_faker($playerInfo->getUsername()));
			}
		}
